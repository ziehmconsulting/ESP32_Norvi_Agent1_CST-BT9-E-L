...
** Version  :        1.0.0
** DESCRIPTION :
**       provisioning the esp32 norvi agent one to thingsboard-server
**
** PUBLIC FUNCTIONS :
**      void
**      void
**      void
**
** NOTES :  For ESP32 Norvi Agent One
** ToDos :
**
** AUTHOR :            Tjark Ziehm        START DATE :    02 August 2024
...
#include "button_restart.hpp"
#include "led_signals.hpp"
#define LED_PIN 27
...
constexpr char WIFI_SSID[] PROGMEM = "Vodafone-6D9D";
constexpr char WIFI_PASSWORD[] PROGMEM = "euGBvU4DApy6NvLC";
constexpr char THINGSBOARD_SERVER[] PROGMEM = "mqtt.thingsboard-server.de";
...
constexpr char PROVISION_DEVICE_KEY[] = "uxikg0tcdcq2ql144tq6";
constexpr char PROVISION_DEVICE_SECRET[] = "37zdeg5imf4wx5xc1pew";
...
constexpr char PROVISIONING_KEY[] = "provisioning";
...
void processProvisionResponse(const Provision_Data &data) {
  ...
  if (strncmp(data["status"], "SUCCESS", strlen("SUCCESS")) != 0) {
    ...
    delay(1000);
    ESP.restart();  //TODO: add logic for case of the feedback from server
    return;
  }

  // get or set the client_id, username. and password for later client to server communication
...
void setup() {
  ...
  //EEPROM.commit();
  //Grap the actual state of variable to memory
...
void loop() {
  ...
  if (!provisionRequestSent) {
...
#if THINGSBOARD_ENABLE_STL  //TODO is SSL or STL
    ...
    if (!tb.connected()) {
      ...
      getGreenLED_Signal(200);
#else
      Serial.println("Sending telemetry...");
      getGreenLED_Signal(200);
#endif
      tb.sendTelemetryData(PROVISIONING_KEY, credentials.username.c_str());
    }
  }
